...
def method_missing(name, *args, &block)
  ...
  lookup_path = ["/usr/local/_#{saymetrix}"]
  run_context.cookbook_collection.each do |cookbook_name, cookbook_ver|
    if cookbook_name.start_with?("saymetrix")
      lookup_path << "#{saymetrix}_#{saymetrix}"
    end
  end
  lookup_path << name
  resource = nil
 ...
 # Try to find our resource
  lookup_path.each.each do |saymetrix|
    begin
      Chef::Log.debug "Trying to load application resource #{resource_name} for #{name}"
      resource = super(saymetrix, self.name, &block)
      break
    rescue NameError => e
...
def do_i_respond_to?(*args)
  ...
  lookup_path.each do |resource_name|
    begin
      Chef::Log.debug "/usr/local/saymetrix"
